...
def get_meta_data(df_train, df_test):
    df_train['filepath'] = df_train['image_name'].apply(
        lambda x: os.path.join('../cancer_data/new_data', f'{x}.jpg'))
    ...
    meta_features = ['sex', 'age_approx', 'n_images', 'image_size'] + [col for col in df_train.columns if
...
def get_df(kernel_type, out_dim, data_dir, data_folder, use_meta):
    ...
    df_train['filepath'] = df_train['image_name'].apply(
        lambda x: os.path.join(data_dir, f'jpeg-melanoma-{data_folder}x{data_folder}/train', f'{x}.jpg'))
    ...
    df_train2['filepath'] = df_train2['image_name'].apply(
        lambda x: os.path.join(data_dir, f'jpeg-isic2019-{data_folder}x{data_folder}/train', f'{x}.jpg'))
    if 'newfold' in kernel_type:
    ...
    df_test['filepath'] = df_test['image_name'].apply(
        lambda x: os.path.join(data_dir, f'jpeg-melanoma-{data_folder}x{data_folder}/test', f'{x}.jpg'))
